...
- (void)connectRongCloud {
    ...
    [[RCIM sharedRCIM] connectWithToken:RONGCLOUD_TOKEN dbOpened:^(RCDBErrorCode code) {
        ...
        // 连接成功后，对当前登录者设置用户信息
        [[RCSDataSource sharedInstance] configCurrentUserInfo:userId];
...
- (void)setRongCloudDelegate{
    ...
    // 设置用户信息提供者代理
    [RCIM sharedRCIM].userInfoDataSource = [RCSDataSource sharedInstance];
    
    // 设置群组信息提供者代理
    [RCIM sharedRCIM].groupInfoDataSource = [RCSDataSource sharedInstance];
}
